...
'BC_E2_Decision':[], 'BC_P4_Decision':[], 'Age':[], 'BMI':[], 'Stimulation':[], 'AMH':[]}

#'BC_E2_Decision_f':[], 'BC_P4_Decision_f':[], 'BC_E2_Decision_1000':[], 'BC_P4_Decision_1000':[]

        patients = data['ID'].unique()

        #STIMULATION AS NUMERIC

        data['Stimulation#'] = data['STIMULATION'].replace({'Antagonists': 1, 'Progestin':2})
        ...
        for p in patients:
            ...
            age = data.loc[data['ID']==p,'AGE'].values[0]
            new_dict['Age'].append(age)
            bmi = data.loc[data['ID']==p,'BMI'].values[0]
            new_dict['BMI'].append(bmi)
            stimulation = data.loc[data['ID']==p,'Stimulation#'].values[0]
            new_dict['Stimulation'].append(stimulation)
            amh = data.loc[data['ID']==p,'AMH'].values[0]
            new_dict['AMH'].append(amh)
    ...
    def synthetics(self, data):
        datapos = data.loc[data['BC_P4_Decision']==1][:5].copy()

        data2 = pd.concat([data, datapos], ignore_index=True)
        return data2
